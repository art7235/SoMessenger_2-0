...
forwardMessage(cid,mid){return this.post(`/chats/${cid}/messages/${mid}/forward`,{target_chat_id:cid})},
changePassword(oldPw,newPw){return this.post('/users/me/password',{old_password:oldPw,new_password:newPw})},
requestPasswordReset(email){return this.post('/auth/reset-password/request',{email})},
verifyPasswordReset(email,code,newPw){return this.post('/auth/reset-password/verify',{email,code,new_password:newPw})},
searchMessages(cid,q){return this.get(`/chats/${cid}/messages/search?q=${encodeURIComponent(q)}`)},

...
def _cors_origins():
    ...
    mobile_origins = ["capacitor://localhost", "http://localhost", "ionic://localhost"]
    if origins:
        return origins + mobile_origins
    return ["*"] + mobile_origins if settings.ENVIRONMENT != "production" else mobile_origins
...
async def security_and_rate_limit(request: Request, call_next):
    if request.url.path.startswith("/api/"):
        ...
        limit = 50 if is_auth else 1000
        window = 60

        key = ("auth" if is_auth else "api", client_ip)
        ...
        if len(bucket) >= limit:
            return JSONResponse(
                {"detail": "Слишком много запросов. Попробуйте позже.", "ip": client_ip},
                status_code=429
            )
        bucket.append(now)
...
async def startup():
    if settings.ENVIRONMENT == "production":
        if not settings.MESSAGE_ENCRYPTION_KEY:
            print("⚠️  MESSAGE_ENCRYPTION_KEY не задан")
        if settings.SECRET_KEY == "somessenger_super_secret_key_2024":
            print("⚠️  SECRET_KEY стандартный")
